...
def test_find_artifacts(xray_scan_instance):
    with patch(
        ...
        xray_scan_instance.find_artifacts(pattern, working_dir, target_dir, excluded_dir)
...
def test_scan_dependencies_success(xray_scan_instance):
    with patch("subprocess.run") as mock_subprocess_run, patch(
        ...
        "builtins.open"
    ) as mock_open, patch("os.path.join") as mock_path_join:
        prefix = "jf"
        target_dir_name = "target_dir"
        working_dir = "/path/to/working_dir"
        bypass_limits_flag = True
        mock_subprocess_run.side_effect = Mock(returncode=0)
        result = xray_scan_instance.scan_dependencies(prefix, target_dir_name, working_dir, bypass_limits_flag)

        mock_subprocess_run.assert_called_with(
            [prefix, "scan", "--format=json", "--bypass-archive-limits", f"{target_dir_name}/"],
...
def test_scan_dependencies_failure(xray_scan_instance):
    with patch("subprocess.run") as mock_subprocess_run, patch(
        ...
        xray_scan_instance.scan_dependencies(prefix, target_dir_name, working_dir, bypass_limits_flag)
...
def test_run_tool_dependencies_sca_linux(xray_scan_instance):
    ...
    with patch("platform.system") as mock_system, patch(
        ...
        result = xray_scan_instance.run_tool_dependencies_sca(
            remote_config, working_dir, skip_flag, scan_flag, bypass_limits_flag, pattern, token
        )
