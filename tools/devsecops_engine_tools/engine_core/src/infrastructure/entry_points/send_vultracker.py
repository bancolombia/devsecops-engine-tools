...
def get_inputs_from_cli(args):
    try:
        ...
        parser.add_argument("-pt", "--personal_access_token", required=False, help="System access token")
        parser.add_argument("-tc", "--token_cmdb", required=False, help="token Cmdb for list evcs")
        parser.add_argument("-tv", "--token_vultracker", required=False, help="token vultracker")
...
def send_vultracker(path_file):
    try:
        (personal_access_token, token_cmdb, token_vultracker) = get_inputs_from_cli(sys.argv[1:])
        enviroment_mapping = {"dev": "Development", "qa": "Staging", "pdn": "Production"}
