...
class FromDictMixin:
    ...
    def to_dict(self):
        if self == {}:
            return self
        available_fields = {
            field.name: field for field in dataclasses.fields(self)}
        transformed_data = {}
        for field_name, field_type in available_fields.items():
            ...
            if isinstance(attribute, list):
                ...
                for key, element in attribute.items():
                    transformed_data[navitaire_key][key] = FromDictMixin.attribute_to_dict(
                        element)
                ...
                transformed_data[navitaire_key] = FromDictMixin.attribute_to_dict(
                    attribute)
        ...
        for key, value in data.items():
            ...
            if internal_key in available_fields.keys() and value:
                ...
                if matching_internal_field.type in built_in_types:
                    ...
                    internal_value = matching_internal_field.type.from_dict(
                        value)
                elif (
                    isinstance(
                        matching_internal_field.type,
                        typing._GenericAlias)
                    and matching_internal_field.type.__origin__ == list
                ):
                    value_class = matching_internal_field.type.__args__[0]
                    internal_value = []
                    if hasattr(value_class, "from_dict") and callable(
                            value_class.from_dict):
                        internal_value = [
                            value_class.from_dict(v) for v in value]
                    else:
                    ...
                    if hasattr(value_class, "from_dict") and callable(
                            value_class.from_dict):
                        internal_value = {
                            k: value_class.from_dict(v) for k, v in value.items()}
                    else:
                        internal_value = value
                else:
                    internal_value = None
