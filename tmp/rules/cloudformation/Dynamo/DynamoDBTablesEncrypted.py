    ...
    def scan_resource_conf(self, conf):
        ...
        if properties is not None:
            ...
            if sse_config is not None:
                ...
                sse_type = sse_config.get('SSEType')
                if sse_enabled and (sse_key is not None and is_not_empty(sse_key)) and (sse_type is not None and is_not_empty(sse_type)):
                    return CheckResult.PASSED
        return CheckResult.FAILED

    def get_evaluated_keys(self) -> List[str]:
        return ["Properties/SSESpecification/SSEEnabled", "Properties/SSESpecification/KMSMasterKeyId", "Properties/SSESpecification/SSEType"]
