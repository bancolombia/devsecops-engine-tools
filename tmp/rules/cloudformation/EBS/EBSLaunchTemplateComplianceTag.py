...
class EBSLaunchTemplateComplianceTag(BaseResourceCheck):
    def __init__(self):
        name = "Ensure EBS LaunchTemplate has compliance tag"
        id = "CKV_AWS_249"
                                ...
                                if 'Tags' in item.keys():
                                        for item2 in item['Tags']:
                                            if 'Key' in item2.keys() and 'Value' in item2.keys():
                                                ...
                                                if item2['Key'] in verified_tag("cumplimiento") and verified_listValues(listValues, item2['Value'].split("-")):
                                                    return CheckResult.PASSED
...
check = EBSLaunchTemplateComplianceTag()