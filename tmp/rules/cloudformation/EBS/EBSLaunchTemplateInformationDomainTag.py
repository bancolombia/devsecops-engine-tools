...
class EBSLaunchTemplateInformationDomainTag(BaseResourceCheck):
    def __init__(self):
        name = "Ensure EBS LaunchTemplate has information domain tag"
        id = "CKV_AWS_251"
                                ...
                                if 'Tags' in item.keys():
                                        for item2 in item['Tags']:
                                            if 'Key' in item2.keys() and 'Value' in item2.keys():
                                                ...
                                                listValues = ["canales", "productos", "activos", "financiera", "contable", "legal", "riesgos", "seguridad", "no"]

                                                if item2['Key'] in verified_tag("dominio-informacion") and verified_listValues(listValues, item2['Value'].split("-")):
                                                    return CheckResult.PASSED 
        if not flag:
            return CheckResult.PASSED
        else:
...
check = EBSLaunchTemplateInformationDomainTag()