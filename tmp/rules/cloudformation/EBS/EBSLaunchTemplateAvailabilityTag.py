...
class EBSLaunchTemplateAvailabilityTag(BaseResourceCheck):
    def __init__(self):
        name = "Ensure EBS LaunchTemplate has availability tag"
        id = "CKV_AWS_246"
                                ...
                                if 'Tags' in item.keys():
                                        for item2 in item['Tags']:
                                            if 'Key' in item2.keys() and 'Value' in item2.keys():
                                                if item2['Key'] in verified_tag("clasificacion-disponibilidad") and item2['Value'] in ["sin impacto","impacto tolerable","impacto moderado","impacto critico"]:
                                                    return CheckResult.PASSED
...
check = EBSLaunchTemplateAvailabilityTag()