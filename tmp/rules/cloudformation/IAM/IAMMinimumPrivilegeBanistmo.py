...
class IAMMinimumPrivilegeBanistmo(BaseResourceCheck):
    def __init__(self):
        name = "Ensure no IAM policies documents allow \"*\" as a statement's actions and Resources Banistmo"
        id = "CKV_AWS_394"
        supported_resources = [
...
check = IAMMinimumPrivilegeBanistmo()
...
def check_resource(statement):
    if 'Resource' in statement:
        if 'Action' in statement and any(x for x in ["elasticloadbalancing:DescribeAccountLimits", "elasticloadbalancing:DescribeListenerCertificates",
                                                     ...
                                                     "cloudwatch:cloudwatch:PutMetricData", "wafv2:GetChangeToken", "wafv2:GetChangeTokenStatus", "wafv2:ListActivatedRulesInRuleGroup", "wafv2:ListByteMatchSets",
                                                     ...
                                                     "quicksight:UpdatePublicSharingSettings", "quicksight:UpdateResourcePermissions", "logs:CancelExportTask", "logs:CreateLogDelivery", "logs:DeleteAccountPolicy", "logs:DeleteLogDelivery", "logs:DeleteQueryDefinition",
"logs:DeleteResourcePolicy", "logs:DescribeAccountPolicies", "logs:DescribeDeliveries", "logs:DescribeDeliveryDestinations", "logs:DescribeDeliverySources", "logs:DescribeDestinations",
"logs:DescribeExportTasks", "logs:DescribeLogGroups", "logs:DescribeQueries", "logs:DescribeQueryDefinitions", "logs:DescribeResourcePolicies", "logs:GetLogDelivery", "logs:Link",
"logs:ListLogDeliveries", "logs:PutAccountPolicy", "logs:PutQueryDefinition", "logs:PutResourcePolicy", "logs:StartLiveTail", "logs:StopLiveTail", "logs:StopQuery", "logs:TestMetricFilter",
"logs:UpdateLogDelivery", "xray:BatchGetTraceSummaryById", "xray:BatchGetTraces", "xray:DeleteResourcePolicy", "xray:GetDistinctTraceGraphs", "xray:GetEncryptionConfig", "xray:GetGroups", "xray:GetInsight", "xray:GetInsightEvents", "xray:GetInsightImpactGraph", 
"xray:GetInsightSummaries", "xray:GetSamplingRules", "xray:GetSamplingStatisticSummaries", "xray:GetSamplingTargets", "xray:GetServiceGraph", "xray:GetTimeSeriesServiceStatistics", "xray:GetTraceGraph", "xray:GetTraceSummaries", "xray:Link", "xray:ListResourcePolicies", 
"xray:PutEncryptionConfig", "xray:PutResourcePolicy", "xray:PutTelemetryRecords", "xray:PutTraceSegments"] if x in force_list(statement['Action'])):
            return False    
        con = 0
        for x in force_list(statement['Resource']):
            ...
            if bool(re.search("((Fn::Join|Fn::Sub|arn).[a-zA-Z0-9*._:]*.(\\*$)|^[a-zA-Z0-9*_:]*$)", string_val_final)):
                con = con+1
