...
class FSXBaseLineBackupTag(BaseResourceCheck):
    def __init__(self):
        name = "Ensure FSX has base line backup tag"
        id = "CKV_AWS_217"
                    ...
                    for item in conf['Properties']['Tags']:
                        if 'Key' in item.keys() and 'Value' in item.keys():
                            if item['Key'] == "bancolombia:bkmensual":
                                contbkm = contbkm + 1
                                if ("fsx" in str(item['Value']) or "no" in str(item['Value'])): 
                                    contbk = contbk + 1
