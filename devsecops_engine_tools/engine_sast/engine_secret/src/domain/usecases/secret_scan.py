...
class SecretScan:
    ...
    def process(self, dict_args, tool):
        ...
        config_tool, skip_tool = self.complete_config_tool(init_config_tool, tool)
        finding_list = []
        if skip_tool == "false":
            agent_temp_dir = self.devops_platform_gateway.get_variable("temp_directory")
            system_working_dir = self.devops_platform_gateway.get_variable(
                "path_directory"
            )
            exclude_path = config_tool.exclude_path
            agent_os = self.devops_platform_gateway.get_variable("os")
            agent_work_folder = self.devops_platform_gateway.get_variable("work_folder")
